...
/**
 * IO的装饰器使用，其中InputStream就相当于上述的Component接口，只不过这里是一个抽象类，这是我们装饰的目标抽象类。
 * FileInputstream就是一个ConcreteComponent，即待装饰的具体对象，
 * 它并不是JAVA的IO结构中的一个装饰器，因为它无法装饰InputStream。
 * 剩下BufferedInputStream，DataInputstream等等就是各种装饰器了，
 * JAVA的IO中也有抽象的装饰器基类的存在，只是上述没有体现出来，就是FilterInputStream，它是很多装饰器最基础的装饰基类。
 * 
 * 其中dataInputStream是经过两次装饰后得到的，
 * 它具有了dataInputStream和bufferedInputStream的双重功能，
 * 另外，InputStreamReader是一个特殊的装饰器，它提供了字节流到字符流的桥梁，其实它除了具有装饰器的特点以外，也有点像一个适配器. 
 *
 */
